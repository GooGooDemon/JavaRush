...
public class Solution {
    ...
    public static boolean checkTelNumber(String telNumber)
    {
        ...
        if (!telNumber.matches("^[+(\\d].*")) return false; // номер должен начиться с "+", "(" или с цифры
        if (!telNumber.matches(".*\\d$")) return false; // номер должен заканчиваться цифрой
        if (telNumber.matches(".*[^0-9()+-].*")) return false; // номер не должен содержать букв
        if (telNumber.matches(".*--.*")) return false; // номер не должен содержать два тире подряд
        if (telNumber.matches(".*-.*\\(.*")) return false; // тире не должны быть перед скобками
        if (telNumber.matches(".*\\(.*\\(.*")) return false; // двух скобок не может быть
        if (telNumber.matches(".*\\).*\\).*")) return false; // двух скобок не может быть

        // проверяем совпадение шаблонам
        if (telNumber.startsWith("+") && telNumber.replaceAll("\\D", "").length() == 12)
        {
            if (telNumber.matches("\\+\\d+(\\(\\d{3}\\))?\\d+-?\\d+-?\\d+")) return true; // +38(150)123-45-67
            if (telNumber.matches("\\+\\d+(\\(\\d{3}\\))?\\d+-?\\d+")) return true;       // +3812345(150)6-7
            if (telNumber.matches("\\+\\d+(\\(\\d{3}\\))?\\d+")) return true;             // +38123456(150)7
        }
        if (telNumber.replaceAll("\\D", "").length() == 10) {
            if (telNumber.matches("[0-9]*(\\(\\d{3}\\))?\\d+-?\\d+-?\\d+")) return true; // (345)0512027, 3450-512-027
        }
